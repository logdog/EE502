...
// 	TCNT1  = 0;
//
//  // (16MHz / prescaler) = 2000kHz
//  // 2000kHz / 40 = 50 kHz
// 	OCR1A = 39;
//
// 	// CTC mode, prescaler=8, enable output compare interrupt
// 	TCCR1B |= _BV(WGM12) | _BV(CS11) | _BV(OCIE1A);
// 	sei();
//}
...
void updateStepMode(uint8_t mode) {             //  PB2,  PB1,  PB0
  switch(mode){                                 //   M0,   M1,   M2
  case 1: PORTB &= ~0x7; break;                 //  Low,  Low,  Low   Full Step                 working
  case 2: PORTB &= ~0x3; PORTB |= 0x4; break;   //  Low, High, High   Half Step                 working
  case 4: PORTB &= ~0x5; PORTB |= 0x2; break;   // High,  Low, High   Quarter Step              working
  case 8: PORTB &= ~0x1; PORTB |= 0x6; break;   // High, High,  Low   Eighth Step               working
  case 16: PORTB |= 0x7; break;                 // High, High, High   Sixteenth Step            working
...
//  now = micros();
//  nextStepTime = now + timeUntilNextStep(velocity);
}
...
{
	now = micros();
  float dt = (float) ((now - lastTime)*1e-6);
  lastTime = now;
  ...
  // microstepping
  uint8_t stepMode = determineStepMode(velocity);
...
//    digitalWrite(STEPPER_SLEEP_PIN, HIGH);
//    acceleration = 0.075 * (-encoderPos + cartPosition/1000.0);
    acceleration = 0.095 * (-encoderPos);
  }
